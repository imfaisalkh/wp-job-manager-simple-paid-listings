		...
		try {
			...
			$response = wp_remote_post( $this->api_endpoint . 'v1/charges', array(
				...
				'body'       => array(
					...
					'receipt_email' => $this->get_job_listing_author_email( $job_id ),
				),
				'timeout'    => 60,
				'sslverify'  => false,
			...
			if ( ! empty( $parsed_response->error ) ) {
				...
				if ( isset( $parsed_response->balance_transaction ) && ! empty( $parsed_response->balance_transaction->fee ) ) {
					update_post_meta( $job_id, 'Stripe Fee', number_format( $parsed_response->balance_transaction->fee / 100, 2, '.', '' ) );
				}
				// Notify admin
