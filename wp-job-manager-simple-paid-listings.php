...
class WP_Job_Manager_Simple_Paid_Listings {
	...
	public function __construct() {
		...
		define( 'JOB_MANAGER_SPL_VERSION', '1.3.1' );
	...
	public function payment_result_page() {
		get_job_manager_template( 'job-submitted.php', array(
			'job' => $job,
		) );
	}
	...
	public function my_job_do_action( $action = '', $job_id = 0 ) {
		if ( $action == 'pay' && $job_id ) {
			...
			), get_permalink( get_page_by_path( get_option( 'job_manager_submit_page_slug' ) )->ID ) ) );
			exit;
		}
	}
}

$GLOBALS['job_manager_simple_paid_listings'] = new WP_Job_Manager_Simple_Paid_Listings();
